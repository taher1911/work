...
export default {
    ...
    data: () => ({
        // TODO: move headding to store and call at PageHeading with time to read, user,
        heading: {
            title:
    ...
    async asyncData({ $content, params }) {
        ...
        var next = "false";
        var previous = "false";
        const article = await $content("topic", params.id)
            .where({ status: { $in: ["publish", "draft"] } })
            .fetch();
        const articles = await $content("topic")
            .where({ status: { $in: ["publish", "draft"] } })
            .fetch();
        articles.forEach((e) =>
            link.push({
